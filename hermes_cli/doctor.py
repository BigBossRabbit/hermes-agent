...
def run_doctor(args):
    ...
    for subdir_name in expected_subdirs:
        ...
        if subdir_path.exists():
            ...
            if should_fix:
                ...
                check_warn(
                    f"~/.hermes/{subdir_name}/ not found",
    ...
    if soul_path.exists():
        ...
        if lines:
            ...
            check_info(
                "~/.hermes/SOUL.md exists but is empty — edit it to customize personality"
            )
    else:
        check_warn(
            "~/.hermes/SOUL.md not found",
    ...
    if tinker_dir.exists() and (tinker_dir / "pyproject.toml").exists():
        if py_version >= (3, 11):
            try:
                ...
                issues.append(
                    "Install tinker-atropos: uv pip install -e ./tinker-atropos"
                )
        else:
            check_warn(
    ...
    # =========================================================================
    print()
    print(color("◆ Model Compatibility", Colors.CYAN, Colors.BOLD))

    try:
        from agent.model_metadata import get_model_context_length, is_local_endpoint
        from hermes_cli.config import load_config

        _cfg = load_config()
        _model = _cfg.get("model", {}).get("default", "")
        _provider = _cfg.get("model", {}).get("provider", "")
        _base_url = _cfg.get("model", {}).get("base_url", "") or _cfg.get(
            "inference", {}
        ).get("api_base", "")

        if _model and _base_url and is_local_endpoint(_base_url):
            ...
            _MIN_SAFE_CTX = 16384
            if _ctx >= _MIN_SAFE_CTX:
                check_ok(f"Model context length: {_ctx:,} tokens")
            else:
                check_fail(
                    f"Model context length: {_ctx:,} tokens",
                    f"(minimum {_MIN_SAFE_CTX:,} needed — Hermes system prompt is ~8-15K tokens)",
                )
                issues.append(
                    f"Load model '{_model}' with context_length >= {_MIN_SAFE_CTX:,} "
                    f"(current: {_ctx:,})"
                )

            # Check inference model too
            _inf_model = _cfg.get("inference", {}).get("model", "")
            if _inf_model and _inf_model != _model:
                _inf_ctx = get_model_context_length(
                    _inf_model,
                    base_url=_base_url,
                )
                if _inf_ctx < _MIN_SAFE_CTX:
                    check_warn(
                        f"Inference model context: {_inf_ctx:,} tokens",
                        f"(below {_MIN_SAFE_CTX:,} minimum)",
                    )
        elif _model:
            check_info(
                f"Model: {_model} (remote provider — context length managed by provider)"
            )
        else:
    ...
    if github_token:
        ...
        check_warn(
            "No GITHUB_TOKEN",
            "(60 req/hr rate limit — set in ~/.hermes/.env for better rates)",
        )
